...
export default function NodePrefillStatus({ status, prefill, errorMessage, onGenerate, onRegenerate }: Props) {
  if (status === "generating") {
    return (
      <div className="rounded-md border border-violet-500/30 bg-violet-500/5 px-3 py-2 flex items-center gap-2">
  ...
  if (status === "error") {
    return (
      <div className="rounded-md border border-rose-500/30 bg-rose-500/5 px-3 py-2 flex items-start gap-2">
        <AlertCircle className="h-3.5 w-3.5 text-rose-400 shrink-0 mt-0.5" />
        <div className="flex-1 min-w-0">
          <p className="text-xs text-rose-200">Falha ao auto-preencher: {errorMessage ?? "erro desconhecido"}</p>
  ...
  return (
    <div className="rounded-md border border-emerald-500/20 bg-emerald-500/5 px-3 py-1.5 flex items-center justify-between gap-2">
      <div className="flex items-center gap-2 min-w-0 flex-1">
        ...
        <Badge variant="outline" className="text-[9px] border-violet-500/30 text-violet-300 shrink-0">
